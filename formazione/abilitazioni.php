...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$cf_doc || !$cod_corso) {
    ...
    if (isset($_POST['add'])) {
      ...
      $stmt->bind_param("ss", $cod_corso, $cf_doc);
      if ($stmt->execute()) {
...
$abil_query = "
  ...
  FROM Abilitazione a
  JOIN Docente d ON a.docente = d.cf
  JOIN Corso c ON a.corso = c.codice
";
...
$stat_query = "
  SELECT 
    ...
    COUNT(DISTINCT docente) AS docenti_con_abilitazioni,
    COUNT(DISTINCT corso) AS corsi_con_docenti
  FROM Abilitazione
";
