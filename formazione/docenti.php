...
// Query per ottenere tutti i docenti con i nomi corretti delle tabelle
$result = $mysqli->query("
  SELECT d.cf, d.cognome, d.cittaNascita, d.tipo,
         COUNT(DISTINCT a.corso) AS num_abilitazioni,
         COUNT(DISTINCT e.codice) AS num_edizioni,
         GROUP_CONCAT(DISTINCT t.numero SEPARATOR ', ') AS telefoni
  FROM Docente d
  LEFT JOIN Abilitazione a ON d.cf = a.docente
  LEFT JOIN Edizione e ON d.cf = e.docente
  LEFT JOIN Telefono t ON d.cf = t.docente
  GROUP BY d.cf
  ORDER BY d.cognome
");
      ...
      <table class="table table-hover" id="docentiTable">
        ...
        <tbody>
          ...
          <tr data-abilitazioni="<?= $d['num_abilitazioni'] ?>">
            ...
            <td>
              <?php if ($d['telefoni']): ?>
                <?php foreach (explode(', ', $d['telefoni']) as $tel): ?>
